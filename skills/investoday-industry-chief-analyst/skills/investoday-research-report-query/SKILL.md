...
title: "研报数据查询"
version: "1.0.1"
description: "提供今日投资研报相关数据查询，覆盖研究报告列表、研究机构列表、研报舆情、研报向量搜索、股票研报预测评级和预测评级变动。Use when: 用户要查研报、研究报告、机构观点、研报摘要、研报核心内容、研报风险机会、研报向量搜索、行业渗透率/市场份额/产能库存/ASP 等研报片段证据、盈利预测、EPS预测、净利润预测、评级变化、研究机构代码，或上层投研 skill 需要先拉取研报证据。Do not use when: 用户要求直接买卖建议、目标价承诺、个性化投资建议、自动交易、完整投研报告撰写、非研报数据查询，或在缺少股票/行业/机构/分类/查询关键词时要求编造研报结论。"
tags:
  - investoday
  - research-report
  - report-query
...
metadata:
  clawdbot:
    ...
    quality_pipeline:
      ...
      pipelines:
        report_query:
          - report_query_executor
        no_agent_clarify: []
        no_agent_handoff: []
      route_agent_pipelines:
        report_list:
          pipeline: report_query
          required: false
        institution_lookup:
          pipeline: report_query
          required: false
...
- 作为上层行业分析补证时，`report/vector-search` 的 body `query` 只传单个关键词或短关键词短语，例如 `库存`、`渗透率`、`CR5`、`ASP`；行业、公司、日期、机构、topK 只作为过滤条件。
- 作为上层行业首席公司池补证时，可从研报列表、预测评级或向量片段中提取带股票代码/公司名的 `company_pool_candidates` 候选线索；研报候选只能作为文本证据或候选来源，不能单独决定公司池五类入选。
...
| 机构查询 | "高盛的机构代码是多少" "查研究机构列表" |
| 研报舆情 | "看一下比亚迪研报舆情" "研报核心观点和风险是什么" |
...
基础 API 能力、接口参数、请求方法、示例命令和返回字段以 `investoday-api search-api` 或 `investoday-api list` 的确认结果为准。
...
- 机构名称或机构代码。
- 研报栏目分类代码。
- 查询关键词，例如 `渗透率`、`市场份额`、`产能利用率`、`ASP`、`TAM`、`技术路线`。向量搜索 body `query` 只能放关键词或短关键词短语，不放行业名称、股票名称、分析句或缺口说明。
...
- 研报向量搜索：查询关键词 + 股票代码、行业代码、机构代码或分类代码至少一个。
- 股票预测评级：股票代码。
- 评级变动：股票代码。
...
- 股票代码 + 查询范围 + 日期范围。
- 向量搜索建议同时提供过滤范围和关键词清单，例如过滤范围为 `300750`，关键词分别查询 `渗透率`、`市场份额`、`产能`、`库存`。多个关键词必须拆成多次独立向量检索，不把多个问题拼成一句 query。
...
- 用户输入股票名称时，先由 `$investoday-finance-data` 的搜索或基础数据能力解析股票代码；多候选时先追问。
- 用户输入行业名称时，优先由行业查询能力确认行业代码；未能确认行业代码时不直接编造。
- 研报目标价和评级字段只作为研报数据展示，不转化为投资建议。
- 上层传入多个缺口关键词时，逐个关键词单独查询并对研报片段去重；`query` 不承担过滤职责。

### 输入校验

- 股票代码必须可识别为 A 股 6 位代码。
- 研报向量搜索不得只传查询关键词，必须同时具备股票、行业、机构或分类过滤范围。
- 研报向量搜索的 `query` 不得是完整自然语言问题，不得包含“行业怎么看”“帮我分析”“缺口”“请查证据”等分析意图词；正确形式是 `库存`、`订单`、`渗透率`、`CR5`、`ASP` 等关键词。
- 时间范围不得早于接口支持口径；超出范围时使用可支持日期并说明。
- 文本片段仅可作为研报证据，不能直接替代结构化财务字段或公司公告原文。

### 缺失输入处理

- 缺少股票、行业、机构、分类或查询关键词时，先说明缺少哪一项，并给出最小可继续查询条件。
...
```text
从比亚迪研报里分别搜索“海外销量”“ASP”“产能释放”“市场份额”的相关片段。
```
...
| 机构列表 | Markdown 表格 | 条件触发 | `report/institutions` | 无结果则提示未匹配 | 机构名称和机构代码 |
| 研报舆情 | Markdown 表格/要点 | 条件触发 | `research/sentiment` | 无结果则标注缺失 | 核心内容、观点、机会、风险、情绪 |
...
| 预测修正摘要 | Markdown 表格/结构化摘要 | 条件触发 | `report/stock-forecast-ratings` + `report/earnings-forecast-rating-cha` | 可比样本不足则说明缺口 | 近期预测样本上调/下调/持平数量、上调占比和变动原因 |
| 评级变动 | Markdown 表格/要点 | 条件触发 | `report/earnings-forecast-rating-cha` | 无结果则标注 | 盈利预测、评级、目标价变动和原因 |
| `company_pool_candidates` | 结构化列表 | 条件触发 | 研报列表/向量片段/预测评级中可识别的股票对象 | 无可识别公司则不输出 | 公司名、股票代码、`source_refs`、来源研报、发布日期、机构、候选环节或主题、候选依据、用途和数据缺口；仅作为候选线索 |
| `source_refs` | 结构化列表 | 条件触发 | 研报列表/舆情/向量/预测评级/评级变动 | 无法定位具体标题、机构或日期时仅内部标注，不作为上层正式引用来源 | 用户可见最小出处：发布日期、发布机构、研报标题、证据类型、用途；供上层正式报告使用时三要素缺一不可 |
...
| `query` | 向量搜索关键词 | string | 向量搜索必填 | 用户输入或上层缺口关键词包 | 查询时点 | 不能执行向量搜索 | `渗透率` |
| `guid` / `reportId` | 研报标识 | string | 否 | 研报列表/舆情/向量搜索 | 研报发布日期 | 无法追溯具体报告 | `A3E...` |
| `chunk` | 向量搜索片段 | string | 否 | 研报向量搜索 | 研报发布时间 | 无片段证据 | `...海外销量...` |
| `source_refs.title` | 研报标题 | string | 条件必填 | 研报标准输出 | 研报发布时间 | 无法定位具体标题时不得进入上层正式报告引用来源 | `宁德时代深度报告...` |
| `source_refs.publish_date` | 发布日期 | date/string | 条件必填 | 研报标准输出 | 研报发布时间 | 无法定位发布日期时不得进入上层正式报告引用来源 | `2026-02-02` |
| `source_refs.institution` | 发布机构 | string | 条件必填 | 研报标准输出 | 研报发布时间 | 无法定位发布机构时不得进入上层正式报告引用来源 | `XX证券` |
| `source_refs.evidence_type` | 证据类型 | string | 条件必填 | 研报标准输出 | 查询时点 | 未分类 | `forecast_rating/vector_search/report_sentiment` |
| `source_refs.used_for` | 上层用途 | string | 条件必填 | 调用方或本 skill 标注 | 查询时点 | 未标注用途 | `盈利预测/风险提示/渗透率证据` |
| `company_pool_candidates.company_name` | 候选公司名称 | string | 条件必填 | 研报列表/预测评级/向量片段 | 研报发布时间 | 无法识别公司则不输出候选 | `中际旭创` |
| `company_pool_candidates.stock_code` | 候选股票代码 | string | 条件必填 | 研报列表/预测评级/向量片段 | 研报发布时间 | 无法识别代码则标注缺口 | `300308` |
| `company_pool_candidates.source_skill` | 候选来源 Skill | string | 是 | 固定值 | 查询时点 | 不适用 | `investoday-research-report-query` |
| `company_pool_candidates.source_type` | 候选来源类型 | enum | 是 | 研报接口类型 | 查询时点 | 不适用 | `report_list/vector_fragment/forecast_rating` |
| `company_pool_candidates.candidate_basis` | 候选依据 | string/list | 条件必填 | 研报标题、片段或预测样本 | 研报发布时间 | 只能作为弱线索 | `研报标题或片段提及高速光模块放量` |
| `company_pool_candidates.data_gaps` | 候选缺口 | list | 否 | 本 skill 标注 | 查询时点 | 无缺口 | `尚未由财务/估值子 Skill 验证` |
...
以下接口均通过 `investoday-api` CLI 使用，endpoint、工具 ID、方法、参数和返回字段均已通过 `investoday-api list` 或 `investoday-api search-api` 确认；详细字段、示例命令和使用边界见后续分组参考文档。
...
| `vector_search` | 研报向量搜索 | 查询关键词 + 过滤范围 | `report/vector-search` | 研报片段证据 |
...
2. 如果只有股票名称，先通过 `$investoday-finance-data` 的搜索能力解析股票代码；多候选时先追问。
...
6. 需要从研报正文片段中找具体证据时调用 `report/vector-search`，并确保有查询关键词和过滤范围；body `query` 只传单个关键词或短关键词短语，行业/公司/日期/topK 作为过滤条件。
...
10. 当上层请求公司池候选线索时，只从已返回研报记录或片段中提取可识别股票对象，输出 `company_pool_candidates`；该字段必须标注 `data_gaps`，提醒后续还需财务、估值或行业数据验证，不得直接作为五类最终入选依据。
9. 输出时区分“结构化预测评级数据”和“研报文本片段证据”；不得把文本片段当作确定结构化字段。

## 数据获取流程

- Step 0：输入识别。确认股票代码、行业代码、机构代码、分类代码、查询关键词和日期范围。
- Step 1：研报列表。命令 `investoday-api report/research --method POST --body-json '{"stockCode":"{股票代码}","beginDate":"{开始日期}","endDate":"{结束日期}","pageNum":1,"pageSize":20}'`
- Step 2：研究机构。命令 `investoday-api report/institutions --method POST --body-json '{"institutionName":"{机构名称}","pageNum":1,"pageSize":20}'`
- Step 3：研报舆情。命令 `investoday-api research/sentiment --method POST --body-json '{"stockCode":"{股票代码}","beginTime":"{开始时间}","endTime":"{结束时间}","pageNum":1,"pageSize":20}'`
- Step 4：研报向量搜索。命令 `investoday-api report/vector-search --method POST stockCode={股票代码} topK=5 --body-json '{"query":"{关键词}"}'`。正确示例：`{"query":"库存"}`、`{"query":"渗透率"}`、`{"query":"CR5"}`、`{"query":"ASP"}`；禁止示例：`{"query":"通信行业库存和订单怎么看"}`、`{"query":"请查通信行业CR5和市场份额证据"}`。
- Step 5：预测评级。命令 `investoday-api report/stock-forecast-ratings stockCode={股票代码} beginDate={开始日期} endDate={结束日期} pageNum=1 pageSize=100`
- Step 6：预测评级变动。命令 `investoday-api report/earnings-forecast-rating-cha stockCode={股票代码}`

> PowerShell 环境中如果 JSON 引号被拆分，可将 body JSON 内部双引号转义，例如 `--body-json '{\"query\":\"渗透率\"}'`。多个关键词必须多次调用，不合并为逗号长串。

## 业务规则与分支逻辑

| 规则ID | 规则名称 | 触发条件 | 判断逻辑 | 输出影响 | 数据来源 | 优先级 |
|---|---|---|---|---|---|---|
| R-001 | 向量搜索过滤范围必需 | 用户要求语义搜索研报片段 | `query` 关键词必填，且 `stockCode/industryCode/institutionCode/categoryCode` 至少一个 | 缺失时先追问 | `report/vector-search` | P0 |
| R-007 | 向量 query 只传关键词 | 用户或上层 Skill 要补查研报片段 | body `query` 只能是单个关键词或短关键词短语；行业、公司、日期、机构、topK 只作为过滤条件 | 多关键词拆成多次查询并去重 `source_refs` | `report/vector-search` | P0 |
...
| R-006 | 预测修正摘要必须可比 | 输出涉及上调占比、机构评级趋势或预测修正方向 | 优先比较同一研报样本的当前预测值与上次预测值；可比样本不足时只说明缺口，不写成已确认未上调 | 避免把预测样本缺口误写成结论 | `report/stock-forecast-ratings` + `report/earnings-forecast-rating-cha` | P0 |
...
| 向量证据补充 | 用户要渗透率、份额、产能等研报证据 | 将用户问题拆为关键词后逐个向量搜索并生成 `source_refs` | 标注发布日期、发布机构、研报标题和不确定性；缺任一项不得作为上层正式引用来源 | 不替代正式结构化字段 |
| 输入不足 | 缺少过滤范围或查询关键词 | 追问最小输入 | 给出可继续查询的字段 | 不猜测标的和行业 |
...
- 向量搜索对用户输入的长问题必须先拆为关键词再查询；实际传给 body `query` 的内容不得包含行业名、公司名、日期、过滤条件或“怎么看/分析/证据/缺口”等意图词。
- `forecast_rating`、`rating_change`、`report_sentiment` 和 `vector_search` 被上层使用时，必须传递对应 `source_refs`。上层正式报告可用的 `source_refs` 必须同时包含发布日期、发布机构和研报标题；若任一项无法定位，只能在本 skill 查询结果中标注需进一步验证，不作为上层正式引用来源。
...
- 数据请求契约见 [references/investoday-research-report-query-data.md](references/investoday-research-report-query-data.md)
...
| 验收项 | 通过标准 | 验证方式 |
|---|---|---|
| 接口覆盖 | 覆盖 `investoday-finance-data` 研报分组 6 个 endpoint | `investoday-api list 研报` 与数据契约核对 |
...
| TC-002 | 向量搜索 | `从 002594 研报里找 ASP 和产能释放` | 拆成 `ASP`、`产能释放` 两次关键词检索，输出研报片段表、代表性研报出处、弱证据说明 | body `query` 不出现股票名或长句；不把片段改写成确定字段，不展示报告 ID、页码或片段位置 |
...
| TC-006 | 关键词 query | 上层要求查询通信行业库存、订单、ASP、CR5 | 分别以 `库存`、`订单`、`ASP`、`CR5` 作为 body `query`，行业作为过滤条件 | 不出现 `通信行业库存和订单怎么看` 这类长 query |
| TC-007 | 多关键词去重 | 同一研报命中 `市场份额` 和 `市占率` | 合并重复 `source_refs`，保留用途标签 | 不重复堆砌同一研报片段 |
...
- 仅通过 `investoday-api` CLI 查询今日投资金融数据。
- 不记录、不存储用户查询记录。
