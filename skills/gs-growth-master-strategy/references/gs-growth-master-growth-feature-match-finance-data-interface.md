# gs-growth-master-growth-feature-match finance-data 接口文档
...
当前转换未能从 Skill 正文中自动识别明确 endpoint。首次接入数据时，先用 `node scripts/gs-api.js search query=<关键词>` 查询接口，再按本文件的“接口文档补全要求”补充。

## 已识别接口详情

未识别到明确 endpoint。请先执行 `node scripts/gs-api.js search query=<关键词>` 获取接口路径、请求方式、入参、出参和示例后补充。
