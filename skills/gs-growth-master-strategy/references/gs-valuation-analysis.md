# 子 Skill 参考：gs-valuation-analysis

- 原路径：`skills/gs-valuation-analysis/SKILL.md`
...
面向 A 股个股与行业板块的估值分析 skill。核心职责是调用 `$gs-valuation-data` 获取结构化估值数据包，再进行成长匹配、安全边际、行业/板块估值位置和估值方法适配分析。

**核心定位：估值分析编排层。数据获取交给 `$gs-valuation-data`；用户可见内容只呈现估值指标、分析逻辑和数据缺口，不暴露内部技术细节。不要输出直接买卖建议、确定性目标价承诺或无法由数据支撑的估值结论。**
...
- 只查询 PE、PB、PS、PEG、股息率、历史分位等原始数据；此类任务应使用 `$gs-valuation-data`
- 直接给出买入、卖出、持有建议
- 承诺目标价、涨跌幅或未来收益
...
- skill: `gs-valuation-data`（本地子 skill：`skills/gs-valuation-data/SKILL.md`）

运行时需要访问估值数据服务；联网能力已在 `requirements.network_access` 声明。

`requirements.skills` 与 `metadata.clawdbot.requires.skills` 必须同步声明 `gs-valuation-data`。
...
- 未指定市场时，默认按 A 股分析。
- 未指定分析范围时，默认做完整估值分析，但只展示与数据匹配的模块；父级策略 skill 明确指定 `compact_parent_summary` 或 `analysis_depth=compact` 时，只返回核心估值快照，不进入完整估值分析。
...
- 无法识别标的时，要求补充代码或完整名称。
- PEG 分析必须存在有效 PE 与外部提供或上游传入的未来盈利预测；本 skill 不主动获取第三方预测衍生指标。未来预测不可得、预测为负、接近零或来源不清时，PEG 不计算或标注参考价值有限，不得使用诊断 PEG 或历史增速替代。涉及行业 PEG 对比时，必须要求 `$gs-valuation-data` 返回行业名称、行业层级、行业指数 PE/PB、行业估值日期、行业基准预测增速、行业基准 PEG、行业 PEG 口径说明、是否一级回退和不可得原因；行业 PEG 固定优先使用二级行业，二级行业无法形成有效行业基准 PEG 时才回退同体系一级行业。
...
- 缺少分析范围：默认进入完整估值分析；父级策略 skill 明确指定 `compact_parent_summary` 或 `analysis_depth=compact` 时，不按缺少范围处理，直接返回核心估值快照。
...
|----------|----------|----------|----------------|------------|----------|------|
| `full_valuation_analysis` | 完整个股估值分析 | A 股代码或名称 | 可 `Spawn valuation_analysis_executor to analyze valuation`；无 subagent 时当前模型执行 | 读取输出模板、估值方法、PEG、安全边际、行业板块 reference | `$gs-valuation-data` 全量估值数据包 | 综合估值摘要和分模块结论 |
...
| `compact_parent_summary` | 父级 skill 调用摘要 | A 股代码或估值数据包 | 无，当前模型执行 | 不读取完整估值 reference；必要时只读输出规则 | 已传入数据包或 `$gs-valuation-data` 核心估值字段 | PE/PB/PS、市值、估值日期、1 条估值状态、1 个最大缺口 |
...
2. 若用户只要原始数据，转用 `$gs-valuation-data`；若用户要分析，继续本 workflow。
3. 父级 `compact_parent_summary` 只获取核心估值字段：价格、市值、PE、PB、PS、估值日期和必要行业名称；不展开 PEG、DCF、安全边际、行业 PEG 或完整行业分位。
4. 调用 `$gs-valuation-data` 获取所需数据包：
   ...
   - 行业/指数估值：行业名称、行业层级、行业指数 PE/PB、行业估值日期、行业 PE/PB/PS 分位、宽基指数估值；行业 PEG 需要行业预测增速时，消费 `$gs-valuation-data` 或上游行业数据返回的同口径标准输出，并要求返回行业基准预测增速、行业基准 PEG、行业 PEG 口径说明和是否一级回退；不得把不同期限的行业预测字段拼接为行业侧三档 PEG
   - 估值辅助：分红、回购、经营现金流、自由现金流辅助口径、国债收益率
5. 根据意图读取对应 reference；完整分析时按输出模板组合多个模块。
6. 做估值方法适配，先判断主估值方法，再决定是否展开 PEG、安全边际或行业相对估值。
7. 生成结论时必须区分“数据事实”“估值推断”“适用边界”。
8. 输出中性估值状态，不给直接买卖建议。
...
- `$gs-valuation-data` 无法获取数据：说明缺失的数据类型，停止基于该数据继续推断。
...
- 行业 PEG 不可得：必须先确认 `$gs-valuation-data` 已尝试“稳定对比行业 -> 同体系二级行业 -> 行业指数 PE -> 同口径行业基准预测增速”链路；二级行业失败时还必须尝试一级行业回退，且回退后行业 PE 与预测增速必须同属一级口径。不得只因行业分位缺失、行业名称模糊查询失败或未直接返回行业 PEG 就判定不足。若输出行业侧三档 PEG，必须确认底层存在同一预测期限下的高 / 中 / 低预测情景。
...
- 格式：gs 扩展格式，包含 `title`、`version`、`tags`、`metadata.clawdbot`、`requirements`。
- 核心边界：数据查询交给 `$gs-valuation-data`，本 skill 只做估值分析、解释和结构化摘要。

## finance-data 接口文档

当前 Skill 的数据接口口径、参数示例、POST Body 写法和回归检查见 [`gs-valuation-analysis-finance-data-interface.md`](gs-valuation-analysis-finance-data-interface.md)。
